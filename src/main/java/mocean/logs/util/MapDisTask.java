...
public class MapDisTask extends RecursiveTask<Map<String, List<String>>> {
     ...
     * @param end
     */
    public MapDisTask(ArrayList<Audrating> list1, int start, int end, boolean IsStart, String startTime, String stopTime) {
        super();
        this.list = list1;
        this.start = start;
        this.end = end;
    ...
    protected Map<String, List<String>> compute(){
        ...
        if(end - start <THRESHOLD){
            ...
            MapDisTask left = new MapDisTask(list, start, middle, IsStart ,startTime, stopTime);
            MapDisTask right = new MapDisTask(list, middle, end, IsStart, startTime, stopTime);
            //并行执行两个 小任务
            left.fork();
            right.fork();
            //把两个小任务累加的结果合并起来
